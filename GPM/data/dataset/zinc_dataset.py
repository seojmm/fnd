    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        for split in ['train', 'val', 'test']:
            ...
            if split == 'train':
                min = torch.tensor([data.y.min() for data in data_list]).min()
                max = torch.tensor([data.y.max() for data in data_list]).max()
            for data in data_list:
                data.y = (data.y - min) / (max - min)

            self.save(data_list, osp.join(self.processed_dir, f'{split}.pt'))

        self.save(all_data_list, osp.join(self.processed_dir, f'full.pt'))
